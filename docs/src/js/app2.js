...
// Rota para ativação do plugin
app.post('/activate', function(req, res) {
    const { key } = req.body;

    // Verificar se a chave existe e está disponível
    let sql = `SELECT * FROM activation_keys WHERE key_value = ? AND status = 'unused'`;

    conexao.query(sql, [key], function(erro, resultado) {
        if (erro) {
            console.error("Erro ao verificar chave de ativação:", erro);
            return res.json({ success: false, message: "Erro interno no servidor" });
        }

        if (resultado.length === 0) {
            return res.json({ success: false, message: "Chave inválida ou já utilizada" });
        }

        // Se a chave for válida, atualizar status para 'used' e registrar ativação
        let updateSql = `UPDATE activation_keys SET status = 'used' WHERE key_value = ?`;

        conexao.query(updateSql, [key], function(updateErro) {
            if (updateErro) {
                console.error("Erro ao ativar a chave:", updateErro);
                return res.json({ success: false, message: "Erro ao registrar ativação" });
            }

            // Criar arquivo de ativação local
            fs.writeFile(__dirname + '/activation_status.txt', 'activated', (fsErro) => {
                if (fsErro) {
                    console.error("Erro ao criar arquivo de ativação:", fsErro);
                    return res.json({ success: false, message: "Ativação incompleta" });
                }

                res.json({ success: true, message: "Ativação concluída com sucesso!" });
            });
        });
    });
});
